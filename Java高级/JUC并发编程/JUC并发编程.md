...
![线程不安全字节码](图片/共享模型之无锁/线程不安全字节码.png)

- 多线程执行流程

![多线程执行流程1](图片/共享模型之无锁/多线程执行流程1.png)

![多线程执行流程2](图片/共享模型之无锁/多线程执行流程2.png)
...
![compareAndSet](图片/共享模型之无锁/compareAndSet.png)
...
- AQS 用state属性来表示资源的状态（分**独占模式和共享模式**），子类需要定义如何维护这个状态，控制如何获取锁和释放锁
  ...
  - <font color="red">**独占模式是只有一个线程能够访问资源，如 ReentrantLock**</font>
