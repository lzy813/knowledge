...
- <font color="red">**实际上是一个很长的二进制数组（00000000）+ 一系列随机hash算法映射函数，主要用于判断一个元素是否在集合中**</font>
- 通常我们会遇到很多要判断一个元素是否在某个集合中的业务场景，一般想到的是将集合中所有元素保存起来，然后通过比较确定。
- 链表、树、哈希表等等数据结构都是这种思路，但是随着集合中元素的增加，需要的存储空间也会呈线性增长，最终到达瓶颈。同时检索速度越来越慢，上述三种结构的检索时间复杂度分别为O(n)，O(logn)和O(1)。这个时候，就出现了布隆过滤器 



### 6.3 是什么

- 设计思想：由一个初值都为0的bit数组和多个哈希函数构成，用来快速判断集合中是否存在某个元素
- 目的：减少内存占用
- 方式：不保存数据信息，只在内存中做一个是否存在的标记flag

![布隆过滤器](图片/高级/布隆过滤器/布隆过滤器设计思想.png)

- 本质来说：就是判断具体数据是否存在于一个大的集合中
- 布隆过滤器是一种<font color="red">**类似set**</font>的数据结构，只是**统计结果在巨量数据下有点小瑕疵，不够完美**
...
- 优点：高效的插入和查询，占用空间少
- 缺点：返回结果不确定性，有误差
- 重点：<font color="red">**一个元素如果判断结果为存在时，那么元素不一定存在；但是判断结果不存在时，则一定不存在**</font>
  - 理由：比如5号坑位有a，b两个元素映射，而c刚好也映射到了5号坑位，那么这个时候并不能代表c就存在
  - 相反，如果都映射不到哈希表上面，那肯定就是不存在了
- 布隆过滤器可以添加元素，但是<font color="red">**不能删除元素**</font>，由于涉及hashcode判断依据，删掉元素可能导致误判率增加
  - 比如5号坑位有a、b、c三个坑位，想删a，那么连带5号位上的b和c也会被删掉，这样就会增加误判纪律
- 可以保证的是：<font color="red">**如果布隆过滤器判断一个元素不在一个集合中，那这个元素一定不会在集合中**</font>
...
- 基本原理
  ...
  - 实质就是<font color="red">**一个大型位数组和几个不同的无偏hash函数**</font>（无偏表示分布均匀）。由一个初值都为0的bit数组和多个哈希函数构成，用来快速判断某个元素是否存在。但是跟HyperLoglog一样，它有一点不精确，存在一定的误判概率
- 添加key、查询key
  - 添加key：使用多个hash函数对key进行hash运算得到一个整数索引值，对位数组长度取模运算得到一个位置，每个hash函数都会得到一个不同的位置，将这几个位置都置为1就完成了add操作
  - 查询key：只要有其中一位是0就表示这个key不存在，<font color="red">**但如果都是1，则不一定存在对应的key**</font>
...
- 当有变量被加入到集合时，通过N个映射函数将这个变量映射到位图中的N个点，把它们置为1（假定两个变量都通过3个映射函数）

![判断不精确1](图片/高级/布隆过滤器/判断不精确1.png)

- 查询某个变量的时候我们只需要看这些点是不是都为1，就可以大概率知道集合中有没有它了
  - 如果这些点，<font color="red">**有任何一个为0则查询变量一定不存在**</font>
  - 如果都是1，<font color="red">**则只是可能存在**</font>，因为映射函数是散列的，散列函数有碰撞
- 正是基于布隆过滤器的快速检测特性，我们可以在把数据写入到数据库时，用布隆过滤器做个标记。当缓存缺失时，应用查询数据库时，可以通过查询布隆过滤器快速判断数据是否存在。如果不存在，就不用再去数据库中查询了，这样即使发生缓存穿透，大量请求只会查询redis和布隆过滤器，而不会积压到数据库，也就不会影响数据库的正常运行。布隆过滤器可以使用redis实现，本身就能承担较大的并发访问压力
...
![判断不精确2](图片/高级/布隆过滤器/判断不精确2.png)

- 如果两个散列值是不相同的（根据同一函数），那么这两个散列值的原始输入也是不相同的。这个特性是散列函数具有确定性的结果，具有这种特性的散列函数称为单向散列函数
- 散列函数的输入和输出不是唯一对应关系的，如果两个散列值相同，<font color="red">**两个输入值很可能是相同的，但也可能不同**</font>，这种情况称为散列碰撞
...
/**
这里就是两个不同的对象，拥有相同的hash值
2112
2112
 
20w数据就会出现8次的冲突，如果数据量大就更不用说了
运行到第：105362次出现hash冲突，hashcode：2134400190
运行到第：110809次出现hash冲突，hashcode：651156501
运行到第：144993次出现hash冲突，hashcode：2038112324
运行到第：145926次出现hash冲突，hashcode：1164664992
运行到第：151906次出现hash冲突，hashcode：273791087
运行到第：180518次出现hash冲突，hashcode：996371445
运行到第：183831次出现hash冲突，hashcode：254720071
运行到第：186459次出现hash冲突，hashcode：1872358815
 */
~~~
...
- <font color="red">**初始化bitmap**</font>：本质上是由长度为m的位向量或位列表（仅包含0或1位值的列表）组成，最初的所有值均设置为0
- <font color="red">**添加占坑位**</font>：当我们向布隆过滤器中添加元素时，为了尽量地址不冲突，**会使用多个hash函数对key进行运行**，算得一个下标索引值，然后对位数组长度继续宁**取模运算**得到一个位置，每个hash函数都会算得一个不同得位置。再把位数组得几个位置都置为1就完成了add操作。
  - 例如，添加一个字符串wmyskxz，对字符串进行多次hash(key) —> 取模运行 —> 得到坑位

![add操作](图片/高级/布隆过滤器/add.png)

- <font color="red">**判断是否存在**</font>：向布隆过滤器查询某个key是否存在时，先把这个key通过相同的多个hash函数进行运算，查看对应的位置是否都是1
  - <font color="red">**只要有一个为0，那么说明部落过滤器中这个key不存在**</font>
  - <font color="red">**如果这几个位置都是1，那么说明极有可能存在**</font>
  - 比如上面的算出来1、3、5，可能会有冲突，所以即便全部为1，也不一定存在



### 6.8 为什么不要删除

- 布隆过滤器的误判是指多个输入经过哈希之后在相同的bit位置为1了，这样就无法判断是由那个输入产生的
- 因此误判的根源在于相同的bit位被多次映射且置为1
- 这种情况也早就了布隆过滤器的删除问题，因为布隆过滤器的每个bit并不是独占的，很有可能<font color="red">**多个元素共享了某一位**</font>，如果直接删除这一位的话，会影响其他元素
- 特性：<font color="red">**布隆过滤器可以添加元素，但是不能删除元素。因为删除元素会导致误判率增加**</font>
...
- **当实际元素数量远超于初始化数量时，应该对布隆过滤器重新构建，重新分配一个size更大的过滤器，再将历史元素批量add进行**



### 6.10 使用场景

- <font color="red">**解决缓存穿透的问题，和redis结合bitmap使用**</font>

  - 缓存穿透是什么
    - 一般情况下，先查询缓存redis是否有该条数据，缓存中没有时，再查询数据库
    - 当数据库也不存在该条数据时，每次查询都要访问数据库，这就是缓存穿透
    - 缓存穿透带来的问题是，当有大量请求查询数据库不存在的数据时，就会给数据库带来压力，甚至拖垮数据库
  - 可以使用布隆过滤器解决缓存穿透的问题
    - 把已存在数据的key存储在布隆过滤器中，相当于redis前面挡着一个布隆过滤器
    - 当有新的请求时，先到布隆过滤器中查询是否存在
    - 如果布隆过滤器中不存在该条数据则直接返回
    - 如果布隆过滤器已存在，才会查询缓存redis，如果redis没有再查询mysql

  ![解决缓存穿透](图片/高级/布隆过滤器/解决缓存穿透.png)
...
![缓存预热](图片/高级/缓存四大问题/缓存预热.png)



- **缓存预热**（Cache Warm-Up）是指在**系统启动**或**流量高峰来临前**，提前将**热点数据**加载到缓存系统中的过程。
- 为啥需要缓存预热：
...
- 发生原因：
  - **Redis宕机**：偏硬件运维
  - **Redis中缓存的数据大面积同时过期失效**：偏软件开发

![缓存雪崩](图片/高级/缓存四大问题/缓存雪崩.png)

- <font color="red">**会导致大量请求直接到数据库，压垮数据库。**</font>
- 避免和预防
  - **设置有效期均匀分布**
  ...
  - **多缓存结合预防雪崩**
    - ehcache本地缓存 + redis缓存，有些高频的key直接写到本地缓存，这样避免去找redis的麻烦
  - **服务降级**
    - 使用Hystrix或者阿里的sentinel限流&降级，直接让给服务温馨提示
...
- 是什么
  ...
  - 请求去查询一条记录，先查 redis 无，后查 mysql 无，<font color="red">**都查询不到该条记录，但是请求每次都会打到数据库上面去**</font>，导致后台数据库压力暴增，这种现象我们称为缓存穿透，这个 redis 变成了一个摆设

  - 简单说就是，本来无一物，两库都没有。既不在 Redis 缓存库，也不在 mysql，数据库存在被多次暴击风险

![布隆过滤器](图片/高级/缓存四大问题/布隆过滤器.png)

- 解决方案
  - <font color="red">**回写增强**</font>：即回写一个空置或者缺省值放到redis里
    - 流程
      - 如果发生了缓存穿透，我们可以针对要查询的数据，在 Redis 里存一个和业务部门商量后确定的缺省值（比如，零、负数、defaultNull 等）。比如，键 uid:abcdxxx，值 defaultNull 作为案例的 key 和 value
      - 先去 redis 查键 uid:abcdxxx 没有，再去 mysql 查没有获得，这就发生了一次穿透现象。
      - but，可以增强回写机制。**mysql 也查不到的话也让 redis 存入刚刚查不到的 key 并保护 mysql**。
    ...
    - 缺点：黑客会对你的系统进行攻击，拿一个不存在的 id 去查询数据，会产生大量的请求到数据库去查询。**可能会导致你的数据库由于压力过大而宕掉**
  ...
  - <font color="red">**Google布隆过滤器Guava解决缓存穿透**</font>
    - 
...
- 简单来说就是<font color="red">**热点key突然失效了，暴打mysql**</font>
