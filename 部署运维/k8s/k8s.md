...
- 为什么要专门启一个pause来共享给nginx和ghost，而不是用nginx来直接共享给ghost？
  - 因为pause不用干活启动就休眠，nginx一直要给用户访问，所以pause比nginx更加稳定。如果用nginx共享，那么它一宕机，ghost也会跟着宕机
  - 这样设计更加稳定
- nginx不能回收僵尸进程，而pause是专门的，有这个功能
...
- k8s的网络模型假定了所有的Pod都在一个可以<font color="red">**直接连通的扁平的网络空间中**</font>（比如两台机器分别有一个Pod，这两个Pod可以通过对方的地址直接去发送数据，这种情况就是建立在真实的网络之上，可能是覆盖网络，也可能是非覆盖网络）
- 这在GCE（Google Compute Engine）里面是现成的网络模型，k8s假定这个网络已经存在
- 而在私有云里搭建k8s集群，就不能假定这个网络已经存在，需要自己实现这个网络假设，<font color="red">**将不同节点上的docker容器之间的互相访问先打通，再运行k8s**</font>
...
- kubeadm安装
  - <font color="red">**组件通过容器化方式运行。**</font>
  - 优点：简单，自愈（容器有问题，就杀死重启一个）
  - 缺点：掩盖一些细节
- 二进制安装
  - <font color="red">**组件变成系统进程的方式运行。**</font>容器说白了也是系统进程，只不过特殊点，它需要被名称空间所隔离，所以这里所说的组件变成系统进程的方式指的就是纯系统进程
  - 优点：能够更灵活的安装出集群；规模变的更大
  - 缺点：操作难度大
